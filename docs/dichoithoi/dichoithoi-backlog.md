...
| 2 | ⚠️ **CHỐT hướng (nên thêm khối `foodSpots`, tái dùng bảng `products` lọc category) — NHƯNG CHƯA CODE** (sửa 07/2026: dòng này trước đây ghi nhầm "ĐÃ XONG", thực tế grep xác nhận `foodSpots` không có trong `BLOCK_KINDS`/compiler/UI palette nào) | Độ phức tạp compile engine | article-spec §3.1/§10.3 |
| 3 | ⚠️ **CHỐT hướng — NHƯNG CHƯA CODE** (sửa 07/2026: dòng này trước đây ghi nhầm "✅ đã chốt", thực tế 0% prompt AI nào chèn token `[[block:...]]`; cơ chế chèn khối duy nhất đang chạy là thủ công 100% qua `insert-dynamic-block-panel.tsx`). Áp dụng mọi kind — nhưng loại bài `cam-nang` (duy nhất dùng khối động) còn chưa tạo được bằng AI (chỉ "Viết tay"), nên chưa có đường chạy nào để AI gợi ý khối cả. | Độ phức tạp prompt pack | article-spec §10.4, product-spec §7 |
...
| 9 | ⚠️ **CHỐT hướng — NHƯNG CHƯA CODE** (sửa 07/2026: ghi nhầm "✅ đã chốt", thực tế không tìm thấy field/label này ở đâu trong `apps/web`). Nguyên nhân gốc: phụ thuộc form "Tạo bằng AI" cho Article — mà form đó CHƯA TỒN TẠI (chỉ có "Viết tay") — nên ô tư liệu tham khảo chưa có chỗ để đặt vào | Chất lượng bài Article + tín hiệu Who/How/Why | article-spec §1.2 |
