 ...
 * vua noi qua C4). Khong dinh nghia dinh (node) — trang ban do da co san qua
 * GET /destinations/map, chi ghep bang slug.
 */
@Injectable()
export class GetRelationsMapDataUseCase {
  ...
  async execute(): Promise<GetRelationsMapDataResponse> {
    const [clusterDistances, curated] = await Promise.all([
