...
const validArticle: DestinationArticle = {
  ...
  quickFacts: {
    ...
    transport:
      "Từ TP.HCM đi xe khách hoặc máy bay tới sân bay Liên Khương, khoảng 6-7 giờ.",
  ...
  sections: [
    ...
    {
      heading: "Câu chuyện văn hoá - lịch sử Đà Lạt",
      content: longContent("Người Pháp xưa"),
    },
    {
      heading: "Di chuyển tới Đà Lạt",
      content: longContent("Sân bay Liên Khương"),
    },
  ],
  faq: [
    {
      question: "Đà Lạt có gì chơi?",
      answer: "Chợ đêm, hồ Xuân Hương, các vườn hoa nổi tiếng.",
    },
    {
      question: "Nên đi mấy ngày?",
      answer: "2-3 ngày là hợp lý cho chuyến đi trọn vẹn.",
    },
    {
      question: "Đi Đà Lạt tốn bao nhiêu?",
      answer: "Tuỳ dịch vụ, trung bình 2-3 triệu/người/chuyến ngắn.",
    },
  ],
  metadata: {
    name: "Đà Lạt",
    ...
    description:
      "Đà Lạt là thành phố ngàn hoa nổi tiếng của Lâm Đồng, khí hậu mát mẻ quanh năm.",
...
function buildUseCase(
  ...
  const usecase = new ReviewDraftUseCase(
    drafts,
    jobs,
    results,
    reviews,
...
describe("ReviewDraftUseCase — Approve + gate originality (warning khong chan)", () => {
  it("Approve thanh cong khi khong co comparisonKey (gate originality bi bo qua hoan toan)", async () => {
    ...
    expect(getSavedChecks().some((c) => c.gateName === "originality")).toBe(
      false,
    );
  });
  ...
  it("Approve THANH CONG du gate originality fail (similarity vuot nguong) — chi canh bao, khong chan", async () => {
    ...
    expect(job.status).toBe("Approved");
    const originalityCheck = getSavedChecks().find(
      (c) => c.gateName === "originality",
    );
  ...
  it("Approve VAN bi chan khi 1 trong 4 gate loi that (severity error) fail, bat ke gate originality", async () => {
    ...
    const invalidArticle: DestinationArticle = {
      ...validArticle,
      sections: validArticle.sections.slice(0, 2),
    }; // vi pham structure gate: bai diem den can it nhat 3 section
    const draft: DraftRecord = {
      id: randomUUID(),
      jobId: job.id,
      version: 1,
    ...
    const { usecase, savedJobs } = buildUseCase(job, draft, [
      { slug: "thac-pongour", score: 0.9 },
    ]);

    await expect(
      usecase.execute(draft.id, { action: "Approve" }),
    ).rejects.toThrow();
