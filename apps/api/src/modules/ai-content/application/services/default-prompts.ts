...
export const DEFAULT_PROMPTS: Readonly<Record<string, string>> = {
  ...
  "km-bai-viet.section.vi": [
    ...
    `- TAG CMS — cú pháp CHÍNH XÁC: [Type_Param:v1,v2;Param2:v;FlagParam]`,
    ...
    `- CHỈ dùng code (SupplierCode/ProductTag/Url/Code) NGƯỜI DÙNG cấp trong "gợi ý tag" hoặc giữ từ nội dung cũ.`,
    `  KHÔNG tự bịa code; nếu không có code phù hợp thì viết văn xuôi, KHÔNG đặt tag rỗng (CMS sẽ bỏ qua).`,
