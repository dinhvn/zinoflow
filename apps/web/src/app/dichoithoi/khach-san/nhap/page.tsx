...
function parseInput(text: string): HotelImportRow[] {
...
/** Nhap hang loat Khach san tu Google Sheet/CSV/JSON — dry-run xem truoc truoc khi ghi that (hotel-spec §5). */
export default function ImportHotelsPage() {
  ...
  const dryRun = useMutation({
    mutationFn: async () =>
      importHotelsResultSchema.parse(
        await apiSend("POST", "/hotels/import", { items, dryRun: true }),
      ),
  ...
  return (
    <div className="max-w-4xl space-y-5">
      <a href="/dichoithoi/khach-san" className="text-sm text-zinc-500 hover:underline">
        ← Quay lại danh sách khách sạn
      </a>
      <div>
        <h2 className="text-xl font-semibold">Nhập danh sách khách sạn</h2>
      ...
      {apply.data && (
        <div className="rounded border border-emerald-300 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-300">
          ...
          <a href="/dichoithoi/khach-san" className="mt-2 block text-blue-600 hover:underline dark:text-blue-400">
            → Về danh sách khách sạn
          </a>
        </div>
      )}
    </div>
